    ...
    <div className="flex h-full w-full items-center justify-center p-6" onPointerUp={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>
      <div className="flex max-w-sm flex-col gap-4 text-center text-fg">
        ...
        <div className="flex justify-center gap-2">
          <button type="button" className="rounded-md border border-muted/40 px-3 py-2 text-sm" onClick={onBack}>{t("Back to the") + " "}{edge === "end" ? tr("last") : tr("first")}{" " + t("page")}</button>
          {other && (
            <button type="button" className="rounded-md bg-primary px-3 py-2 text-sm font-medium text-white" onClick={onGo}>
