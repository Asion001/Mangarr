...
P9 implemented: a signed-in account can download a standard Mihon `.tachibk` containing only its visible library and reader progress. The export targets the Komga extension's stable primary source ID, embeds the configured server address plus a newly generated revocable device key, and records completed and partial chapter progress. The account UI and Mihon guide explain restore and revocation; protobuf-shape, API, permission, translation and browser coverage verify the flow.

P10 implemented: CI now runs Go race tests against SQLite and PostgreSQL, frontend type and translation checks, unit tests, a production build and the full Chromium suite. Browser failures retain traces, screenshots and an HTML report for seven days. A separate reproducibility job regenerates OpenAPI and TypeScript schemas and fails on committed-file drift before container images can publish. Local Phase 2 validation covered both databases and all ten browser flows; remote workflow results are recorded separately from local evidence.
